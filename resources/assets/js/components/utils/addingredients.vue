...
<script>
    export default {
        ...
        methods: {
            ...
            async getAClass() {
                this.AClass = await axios.get(process.env.API_HOST + `/IngredientsCategory/Get`)
            ...
            async getBClass(ACategoryID) {
                this.BClass = await axios.get(process.env.API_HOST + `/IngredientsCategory/GetByCategoryID/${ACategoryID}`)
                .then(function (response) {
                    const nameList = response.data.map(item => Object.values(item));
                    return nameList;
                })
            ...
            async getBItem(ACategoryID) {
                this.BItem = await axios.get(process.env.API_HOST + `/Ingredients/GetByCategoryID/${ACategoryID}`)
                .then(function (response) {
                    const nameList = response.data.map(item => Object.values(item));
                    return nameList;
                })
            ...
            async getCItem(BCategoryID) {
                this.CItem = await axios.get(process.env.API_HOST + `/Ingredients/GetByCategoryID/${BCategoryID}`)
                .then(function (response) {
                    const nameList = response.data.map(item => Object.values(item));
                    return nameList;
                })
